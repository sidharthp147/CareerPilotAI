...
function ApplicationDetails() {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(
      `ws://${import.meta.env.VITE_API_URL}/ws/recruiter/${user_id}`
    );
  ...
  const handleApprove = async (userId, jobId) => {
    try {
      ...
      setRankedCandidates((prev) =>
        prev.filter((r) => r.job_seeker_id !== userId)
      );
    } catch (error) {
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        >
          {rankCandidates ? "Ranking Candidates..." : "Rank Candidates"}
        </button>
      </div>
            ...
            {rankedCandidates.map((item, index) => (
              ...
              >
                <div className={styles.cardTop}>
                  <h3>
                    #{index + 1}{" "}
                    {item.resume.full_name?.replace(/"/g, "")}
                  </h3>
                ...
                <div className={styles.details}>
                  <p>
                    <strong>Role:</strong> {item.resume.role}
                  </p>
        ...
        {job?.applications?.map((item) => (
          <div key={item.id} className={styles.card}>
            ...
            <div className={styles.details}>
              ...
              <p>
                <strong>Skills:</strong> {item.seeker?.skills}
              </p>
            </div>
            ...
            {item.application?.status === "APPLIED" && (
              <div className={styles.actions}>
                <button
                  ...
                  onClick={() =>
                    handleApprove(
                      item.application.job_seeker_id,
                      id
                    )
                  }
                >
